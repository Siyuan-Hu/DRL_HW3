...
class A2C(object):
    ...
    def load_models(self, model_config_path, num_observation, num_action, actor_lr, critic_lr, model_step):
        self.actor_model = Actor(model_config_path, num_action, actor_lr, model = "./models/actor-"+str(model_step))
...
class Actor(object):
    ...
    def save_model(self, step):
        ...
        self.sess.graph.add_to_collection("state_input", self.input)
        self.sess.graph.add_to_collection("action_output", self.output)
        self.sess.graph.add_to_collection("updata_weights", self.updata_weights)
        self.sess.graph.add_to_collection("model", self.model)

        self.sess.graph.add_to_collection("G", self.G)
        self.sess.graph.add_to_collection("action", self.action)
        saver.save(self.sess, "./models/actor", global_step = step)
    ...
    def load_model(self, model_file):
        ...
        self.updata_weights = graph.get_collection("updata_weights")[0]
        self.input = graph.get_collection("state_input")[0]
        self.output = graph.get_collection("action_output")[0]
...
def main(args):
    ...
    a2c = A2C(env, model_config_path, lr, critic_lr, num_episodes, N_step, render)#, model_step = 4400)
